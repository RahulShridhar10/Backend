...
PROJECT_DIR = os.path.join(BASE_DIR, "utils")
WEIGHTS_PATH = os.path.join(PROJECT_DIR, "unet_best_weights.pth")
MODEL_PATH = os.path.join(PROJECT_DIR, "FSRCNN_x4.pb")
SCRIPT_PATH = os.path.join(PROJECT_DIR, "super_res_mod.py")
...
# Bicubic Upsampling
def bicubic_upsample(input_image: np.ndarray):
    return cv2.resize(input_image, (input_image.shape[1] * modelscale, input_image.shape[0] * modelscale), interpolation=cv2.INTER_CUBIC)

# Super Resolution Function
def super_res(input_image: np.ndarray):
    return sr.upsample(input_image)

# Function for Binarization
def run(input_path, weights_path, device):
    output_file = input_path[:input_path.rfind('.')] + '_output.jpg'
    ...
    x_padded = np.pad(x, ((0, pad_height), (0, pad_width), (0, 0)), mode='constant')
    x_tensor = transforms.ToTensor()(x_padded.copy())
    x_tensor = torch.unsqueeze(x_tensor, 0).to(device)
    
    y_hat = model(x_tensor)
    out_img_padded = y_hat.cpu().data
    out_img_padded = 1 - (out_img_padded - out_img_padded.min()) / (out_img_padded.max() - out_img_padded.min())
    out_img = out_img_padded[:, :, :original_height, :original_width]
...
def predict():
    ...
    filename = os.path.join(UPLOAD_FOLDER, "input_2.jpg")
    file.save(filename)
    ...
    try:
        output_file = run(filename, WEIGHTS_PATH, device)
...
def enhance():
    ...
    file = request.files["file"]
    print("Uploaded File:", file.filename, "Content Type:", file.content_type)
    ...
    try:
        ...
        enhanced_image = super_res(input_image)

        # Save the enhanced image
        output_file = os.path.join(UPLOAD_FOLDER, "enhanced_" + file.filename)
        cv2.imwrite(output_file, enhanced_image)

        print("Enhancement successful, sending file...")
        return send_file(output_file, mimetype="image/jpeg")
...
def recognize():
    print("Received /recognise request")
    print("Request Headers:", request.headers)
    print("Request Files:", request.files)
    ...
    file = request.files["file"]
    input_filename = os.path.join(BASE_DIR, "053.png")
    output_filename = os.path.join(BASE_DIR, "53_processed.png")  # This is the actual processed file

    # Save the uploaded file
    file.save(input_filename)
    print(f"File saved as {input_filename}")
    ...
    try:
        ...
        if os.path.exists(output_filename):
            print("Recognition successful, sending processed file...")
...
def super_resolve_image(input_image_path, output_folder):
    ...
    """
    # command = [
    #     r"C:\Users\Rahul\OneDrive\Desktop\Reader\Palm_Leaf_Reader\Backend\venv\Scripts\python.exe", 
    #     r"C:\Users\Rahul\OneDrive\Desktop\Reader\Palm_Leaf_Reader\Backend\utils\super_res_mod.py",
    #     "-m", r"C:\Users\Rahul\OneDrive\Desktop\Reader\Palm_Leaf_Reader\Backend\utils\FSRCNN_x4.pb",
    #     "-i", input_image_path,
    #     "-o", output_folder
    # ]
...
if __name__ == "__main__":
    MODEL_PATH = os.path.join(PROJECT_DIR, "FSRCNN_x4.pb")  # Update this with the actual model path
    if os.path.exists(MODEL_PATH):
        init_super_res(MODEL_PATH)
    else:
        print("Super-resolution model file missing!")
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    print(BASE_DIR)
